...
def redeemed_cell(credit: dict[str, Any]) -> str:
    ...
    if redeemed_at:
        return f"Yes ({redeemed_at.date().isoformat()})"
    if redeem_started_at:
        return f"Started ({redeem_started_at.date().isoformat()})"
    return "No"
...
def render_table(credits: list[dict[str, Any]], now: datetime) -> str:
    ...
    rows = []
    for credit in sorted(credits, key=lambda item: date_cell(item.get("expires_at"))):
        rows.append(
            [
                available_cell(credit, now),
                status_cell(credit, now),
                credit.get("title", ""),
                date_cell(credit.get("granted_at")),
                date_cell(credit.get("expires_at")),
